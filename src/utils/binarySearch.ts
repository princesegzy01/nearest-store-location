...
const getClosestZip = (zipArray: number[], target: number): number[] => {
	// Declare low and high location
	let low = -1;
	let high = zipArray.length;
	...
	const lowDiff = Math.abs(zipArray[low] - target);
	const highDiff = Math.abs(zipArray[high] - target);
